...
export const CREATIVE_VISIBLE_SUGGESTION_COUNT = 6;
export const CREATIVE_MAX_SUGGESTION_COUNT = 12;
...
const BASE_QUESTIONS: CreativeQuestion[] = [
  ...
  },
  {
    ...
    allowedToolTypes: ["metier", "core", "bundle", "satellite"],
    priority: 100,
  },
  {
  ...
  },
  {
    ...
    allowedToolTypes: ["metier", "core", "satellite", "ia", "specialise"],
    priority: 100,
  },
  {
    ...
    allowedToolTypes: ["metier", "plugin", "satellite", "ia"],
    priority: 85,
  },
  {
    ...
    allowedToolTypes: ["metier", "core", "plugin", "satellite"],
    priority: 100,
  },
  {
    id: "photo-retouch",
  ...
  },
  {
    ...
    allowedToolTypes: ["metier", "core", "satellite", "ia"],
    priority: 100,
  },
  {
    id: "video-finish",
  ...
  },
  {
    ...
    allowedToolTypes: ["metier", "core", "satellite", "plugin"],
    priority: 100,
  },
  {
    ...
    allowedToolTypes: ["metier", "core", "ia"],
    priority: 100,
  },
  {
    id: "three-d-creation",
  ...
  },
  {
    ...
    needKeys: ["rendu-3d", "render-engine", "rendering", "temps-reel", "3d"],
    explicitToolIds: [
      "enscape",
      "twinmotion",
      "lumion",
  ...
  },
  {
    ...
    allowedToolTypes: ["metier", "core", "satellite", "specialise", "ia"],
    priority: 100,
  },
  {
  ...
  },
  {
    ...
    allowedToolTypes: ["ia", "satellite", "metier", "specialise"],
    priority: 66,
  },
  {
    id: "creative-assets",
...
export function rankToolsForCreativeQuestion(
  ...
  return tools
    .map((tool): RankedCreativeTool | null => {
      ...
      } else if (tool.force_silence || tool.prescription_quality === "silence") {
        score -= 72;
      }
...
function creativeSuggestionFamily(tool: Tool): string {
  const normalizedId = normalize(tool.id);
  if (normalizedId.startsWith("adobe-") || normalizedId === "indesign") return "adobe";
  if (normalizedId.startsWith("figma")) return "figma";
  if (normalizedId.startsWith("canva")) return "canva";
  if (normalizedId.startsWith("affinity")) return "affinity";
  if (normalizedId.startsWith("topaz")) return "topaz";
  if (["davinci-resolve", "fusion"].includes(normalizedId)) return "blackmagic";
  if (["cinema-4d", "redshift"].includes(normalizedId)) return "maxon";
  if (["blender", "cycles"].includes(normalizedId)) return "blender";

  return normalize(
    tool.commercial_family ||
      tool.provider_id ||
      tool.bundle_parent ||
      tool.host_app ||
...
function creativeSuggestionTypeBucket(tool: Tool): string {
  if (tool.tool_type === "ia" || tool.ia_use_case) return "ia";
  if (tool.host_app) return `host:${normalize(tool.host_app)}`;
  return tool.tool_type || "unknown";
}

export function diversifyRankedCreativeTools(
  question: CreativeQuestion,
  rankedTools: RankedCreativeTool[],
  limit = CREATIVE_MAX_SUGGESTION_COUNT
): RankedCreativeTool[] {
  if (question.kind === "ecosystem") return rankedTools.slice(0, limit);

  const picked = new Set<string>();
  const result: RankedCreativeTool[] = [];
  const familyCounts = new Map<string, number>();
  const typeCounts = new Map<string, number>();
  const familyCap = question.id === "creative-ai" ? 4 : 2;
  const typeCap = question.id === "creative-ai" ? limit : 5;

  const tryPick = (item: RankedCreativeTool, relaxed = false) => {
    if (picked.has(item.tool.id) || result.length >= limit) return;
    const family = creativeSuggestionFamily(item.tool);
    const type = creativeSuggestionTypeBucket(item.tool);
    const currentFamilyCount = familyCounts.get(family) || 0;
    const currentTypeCount = typeCounts.get(type) || 0;

    if (!relaxed && (currentFamilyCount >= familyCap || currentTypeCount >= typeCap)) return;

    picked.add(item.tool.id);
    result.push(item);
    familyCounts.set(family, currentFamilyCount + 1);
    typeCounts.set(type, currentTypeCount + 1);
  };

  rankedTools.forEach((item) => tryPick(item));
  rankedTools.forEach((item) => tryPick(item, true));
  return result;
}
