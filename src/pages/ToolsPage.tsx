 ...
 */
function hasFreePlan(tool: ToolSummary): boolean {
...
function knownPrice(tool: ToolSummary): number | null {
...
function isPaid(tool: ToolSummary): boolean {
...
const ToolsPage = () => {
  ...
  const shelves = useMemo(() => {
    ...
    return sortedCategories.slice(0, 6).map((category) => {
      ...
      const ranked = tools
        ...
        .sort((x, y) => (demand[y.slug || y.id] || 0) - (demand[x.slug || x.id] || 0) || (x.name ?? "").localeCompare(y.name ?? ""))
  ...
  const filtered = useMemo(() => {
    ...
    result.sort((a, b) => {
      if (sort === "popular") {
        // Real search demand, not fiche maturity (see the need shelves).
        const demand = (toolDemand as { impressions: Record<string, number> }).impressions;
        return (demand[b.slug || b.id] || 0) - (demand[a.slug || a.id] || 0) || (a.name ?? "").localeCompare(b.name ?? "");
      }
