# Artefact 5/6 — Projections publiques (**NON EXÉCUTÉ**)

> Rév. 4. Vue **`security_barrier`** dans `catalog_api`, possédée par `catalog_owner` (NOLOGIN, sans BYPASSRLS). Anon/authenticated lisent **la vue seule** ; les resolvers qu'elle appelle sont `SECURITY DEFINER` avec `EXECUTE` accordé à `anon, authenticated` (A4).
>
> **Rév. 4.9 — Diagnostic différé.** Le déploiement actuel ne crée ni vue, ni grant, ni migration Diagnostic. Les champs potentiellement réutilisables restent stockés sans être exposés par une projection dédiée. Restent publics uniquement les champs réellement consommés par Fiche, Ma Stack, Explorer ou build : `prescription_quality`, `functional_needs`, `verticals`, `host_app`, `bundle_parent`, `substitution_cluster_v2`. L'audit actualisé ajoute les trois blocs actifs omis auparavant : `gallery_images`, `ai_angle`, `pricing_guidance`.
> Union **réelle** des champs consommés par Fiche, Ma Stack, Explorer et build (relevé sur le code, 2026-07-16). Relations legacy restituées.

## Champs consommés (union relevée sur le code)

- **Fiche** (`ToolDetailPage` + `tool/*`) : slug, name, logo, category, tool_type, websiteUrl, affiliateLink, short/longDescription(+En), pricing/pricingEn, pricing_v5→prix comparatif **et contenu de présentation** (cost table/cautions/TCO), verdict(+En), pros, cons, covers, useCases(+En), relevantFor, solo/teamRelevance, personas, seo, `gallery_images`, `aiAngle`, timeGainedHoursPerMonth, alternatives/betterAlternative/freeAlternative/host_app/bundle_parent→relations, articles.
- **Ma Stack** (selector/scoring) : id, name, **prix comparatif** (dérive `price`/`priceCurrency`/`catalogMonthlyPrice`/`selectedOffer`), is_free, tool_type, category. **Ne stocke que `tool_id` + données utilisateur** (`monthlyCost_override?`, `usage`).
- **Explorer / listings** (`ToolSummary`) : id, slug, name, categoryId, shortDescription(+En), pricing, defaultMonthlyPrice→compare, affiliateLink, websiteUrl, logo.
- **Build / prerender** (`vite.config` `getMergedTools`) : slug, name, defaultMonthlyPrice→compare, shortDescription(+En), longDescription(+En), pricing.free/paid, verdict.keepIf, websiteUrl/affiliateLink, seo.

→ La projection expose l'**union** de ces champs, avec un contrat de sortie stable pour les 4 surfaces + le build.

| Groupe de sortie | Fiche | Ma Stack | Explorer | Build/SSR |
|---|:---:|:---:|:---:|:---:|
| `id/tool_id, slug, name, category, logo, og_image_url, website_url, affiliate_link, tool_type` | ✓ | ✓ | ✓ | ✓ |
| `short_description, long_description, verdict, pros, cons, covers, use_cases, relevant_for, seo`, galerie, angle IA et guidance pricing localisés | ✓ |  | ✓ (court) | ✓ |
| `plans, is_free, compare_*`, engagement, taxe, fraîcheur, source | ✓ | ✓ | ✓ (résumé) | ✓ |
| `relationships, alternatives`, pertinence, migration/downgrade | ✓ | ✓ | ✓ | ✓ |
| `prescription_quality`, clusters, besoins, verticales, host/bundle | quelques blocs fiche | ✓ | filtres |  |

Les champs TypeScript camelCase sont adaptés à la frontière (`website_url→websiteUrl`, etc.). La base ne duplique pas ces alias. Les champs `legacy_*` restent disponibles uniquement pendant la bascule ; les nouveaux consommateurs utilisent `plans` et `compare_*`.

## Projection publique commune
...
reset role;
```
> Sortie : **1 ligne par (tool_id, lang)**. Le resolver de prix garantit une ligne ; `uq_editorial_published` empêche la multiplication par contenu. Les champs `legacy_*` sont transitoires, clairement nommés et ne peuvent pas être confondus avec une observation native.

## Frontière applicative obligatoire

Le code actuel consomme encore la forme TypeScript historique (`categoryId`, `shortDescription`, `defaultMonthlyPrice`, `pricing`, etc.). La migration ne doit pas renommer les colonnes SQL ni réintroduire ces doublons dans la base : un adaptateur unique transforme la ligne de projection à la frontière.

Règles bloquantes pour cet adaptateur :

- toute requête filtre explicitement `slug/tool_id` **et** `lang` ; `maybeSingle()` sans langue est interdit puisque la vue contient deux lignes par outil ;
- `category → categoryId`, les champs éditoriaux localisés → propriétés camelCase existantes ;
- `compare_monthly_eur` alimente l'affichage comparatif, mais `price_status` et `is_free` restent deux propriétés distinctes ;
- `price_status='needs_review'` ne devient jamais `defaultMonthlyPrice=0` avec une sémantique « gratuit » ; il reste **prix inconnu** ;
- `is_free=true` indique l'existence d'un plan gratuit durable, indépendamment du plan comparatif ;
- `legacy_pricing*` n'est lu que pour une ligne `data_contract='legacy'` ; une fiche canonical utilise `plans`, `compare_*`, `is_free` et `price_status` ;
- `gallery_images → gallery_images`, `ai_angle → aiAngle` ; `pricing_guidance` ne reconstitue que les éléments éditoriaux de `pricing_v5`, tandis que les champs factuels viennent obligatoirement de `compare_*`, `plans`, fraîcheur et source ;
- `price_last_confirmed_on` (fallback `price_observed_on`) alimente la date publique `pricing_v5.verified_on`, le JSON-LD `dateModified` et les libellés « vérifié le » ; un simple statut `fresh/stale` ne remplace jamais cette date ;
- `relationships` est la source structurée commune ; `alternatives` n'est qu'un dérivé de compatibilité ;
- pendant la transition applicative, l'adaptateur dérive aussi `relations`, `complements` et `integrates_with` depuis `relationships` pour les composants historiques ; aucune seconde relation n'est stockée en base ;
- Fiche, Ma Stack, Explorer et build utilisent le même adaptateur de contrat, sans recalcul divergent du prix ou de la gratuité.

Le type applicatif devra donc porter au minimum `catalogIsFree`, `catalogPriceStatus`, `catalogPlans` et les champs `compare*` pendant la bascule. Le retrait des anciens champs n'intervient qu'après parité sur les 1 126 outils.

## Diagnostic — explicitement différé

Aucun objet `catalog_api.diagnostic_tool_projection` n'est créé dans cette révision. Aucun droit n'est accordé et aucune bascule de `useDiagnosticData` n'entre dans le chemin critique. Les champs historiques sont conservés pour ne pas fermer la porte à un usage futur, mais leur activation exigera une décision dédiée, un relevé actualisé des consommateurs et un nouvel artefact de projection.
