      ...
      },
      {
        ...
        acceptedAnswer: {
          ...
          text: (() => {
            ...
            if (priceDisplay) return `${name} offers ${hasFree ? "a free plan, then paid tiers from" : "paid plans from"} ${usd(tool, priceDisplay)}/month. Price verified by ToolTrim.`;
      ...
      try {
        ...
        const SEO_PAGES: { path: string; title: string; description: string; bodyText: string }[] = [
          {
            ...
            bodyText: "La stack d'un développeur freelance se construit vite et se nettoie rarement. Les postes qui pèsent le plus : un IDE avec IA, l'hébergement, les abonnements LLM. Le gaspillage vient rarement d'un outil de trop, il vient de deux outils qui font la même chose.",
          },
          {
            ...
            bodyText: "Le budget outils d'un designer freelance part rarement là où il croit. Les postes à surveiller : un abonnement Adobe complet quand deux applications suffisent, deux banques d'images en parallèle, et des plugins achetés puis jamais rouverts.",
          },
          {
            ...
            bodyText: "Un consultant a peu d'outils mais les paie cher, parce que ce sont des outils de vente. Le piège principal reste le cumul de deux CRM, l'un gardé par habitude, l'autre adopté pour une fonction précise puis jamais abandonné.",
          },
          {
            ...
            bodyText: "La stack d'un créateur de contenu grossit par empilement : un outil IA ajouté pour un cas précis, un scheduler testé puis gardé, une plateforme newsletter jamais migrée. Trois familles concentrent l'essentiel des doublons.",
          },
          {
            ...
            bodyText: "La stack d'un ops manager ou d'un COO à temps partagé est la plus fragmentée de toutes : compta, banque, signature électronique, gestion de projet, stockage. Plus il y a de familles d'outils, plus les recouvrements passent inaperçus.",
          },
          {
            ...
            bodyText: "A freelance developer's stack gets built fast and cleaned rarely. The heaviest line items are an AI-enabled IDE, hosting and LLM subscriptions. Waste rarely comes from one tool too many, it comes from two tools doing the same thing.",
          },
          {
            ...
            bodyText: "A freelance designer's tool budget rarely goes where they think it does. The line items worth checking: a full Adobe plan when two apps would do, two stock libraries running in parallel, and plugins bought once and never reopened.",
          },
          {
            ...
            bodyText: "A consultant runs few tools but pays a lot for them, because they are sales tools. The main trap is running two CRMs at once, one kept out of habit and one adopted for a single feature then never dropped.",
          },
          {
            ...
            bodyText: "A content creator's stack grows by accretion: an AI tool added for one case, a scheduler trialled then kept, a newsletter platform never migrated away from. Three families account for most of the overlap.",
          },
          {
            ...
            bodyText: "An ops manager's or fractional COO's stack is the most fragmented of all: accounting, banking, e-signature, project management, storage. The more tool families there are, the easier overlaps are to miss.",
          },
          {
            ...
            description: "Loom Business costs $15 per user per month in 2026. When the paid plan earns its place, when the free one is enough, and four cheaper alternatives.",
            bodyText: "Loom Business at $15 per user per month only earns its place if you record often enough to replace written updates. Below that, the free plan covers the occasional walkthrough. Tella, Claap and Scribe cover the core of what most freelancers actually use Loom for, at a lower price.",
          },
          {
            ...
            description: "Loom Business coûte 15$ par utilisateur et par mois en 2026. Quand le plan payant se justifie, quand le gratuit suffit, et quatre alternatives moins chères.",
            bodyText: "Loom Business à 15$ par utilisateur et par mois ne se justifie que si tu enregistres assez souvent pour remplacer tes comptes rendus écrits. En dessous, le plan gratuit couvre la démonstration occasionnelle. Tella, Claap et Scribe couvrent l'essentiel de ce que la plupart des freelances font avec Loom, pour moins cher.",
          },
          {
            ...
            description: "Grammarly Premium costs $12/mo against $4/mo for LanguageTool. What each one actually corrects, and which is worth paying for.",
            bodyText: "Grammarly Premium costs $12 a month against $4 a month for LanguageTool Premium. Both free tiers cover routine correction. LanguageTool is the better pick for multilingual writing, and Grammarly earns its premium mainly if you write high volumes of English for native-English clients.",
          },
          {
            ...
            description: "Stack IA minimale, prompts par métier et chasse au gaspillage. Combien d'abonnements IA se justifient vraiment, et lequel garder.",
            bodyText: "En 2026, un freelance a besoin d'un seul abonnement IA généraliste, ChatGPT Plus ou Claude Pro, plus éventuellement un outil métier spécialisé. Cumuler ChatGPT, Claude, Perplexity et Copilot revient à payer plusieurs fois la même capacité de rédaction. Le test qui tranche : sur les sept derniers jours, lequel as-tu vraiment ouvert ?",
          },
          {
          ...
          },
          {
            ...
            description: "Minimal AI stack, role-based prompts and a waste check. How many AI subscriptions actually earn their place, and which one to keep.",
        ...
        for (const post of allPostsData) {
          ...
          const articleSchema = {
            ...
            image: [post.thumbnail ? `${BASE}${post.thumbnail}` : `${BASE}/og-image.png`],
          ...
          if (renderGuidePage) {
            try {
              const markup = await renderGuidePage(`/${lang}/${routeKind}/${slug}`, post);
              html = html.replace('<div id="root"></div>', `<div id="root">${markup}</div>`);
              if (compiledCssPath) {
                const utilityCss = extractUsedUtilityCss(markup, compiledCssPath);
                if (utilityCss) {
          ...
          const outDir = path.resolve(distDir, lang, routeKind, slug);
          fs.mkdirSync(outDir, { recursive: true });
          fs.writeFileSync(path.resolve(outDir, "index.html"), html, "utf-8");
          count++;
        }
    ...
    rollupOptions: {
      output: isSsrBuild ? undefined : {
        manualChunks(id) {
          ...
          if (id.includes("/src/data/stacks-catalog-index.json")) return "data-stack-index";
