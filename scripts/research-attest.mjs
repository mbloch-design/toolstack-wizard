  ...
  market_context: {
    reference_fr: {
      ...
      verify(ctx) {
        ...
        // Le faisceau FORT (egress FR + playwright + fr-FR ×3 + Europe/Paris + EUR présent) établit
        ...
        // Les symboles non-EUR sont des add-ons MONDIAUX facturés en $ (crédits IA, domaines…) HORS
        // grille de sièges : ils sont consignés en corroboration et n'invalident pas le marché EUR
        // des sièges — seul objet de l'attestation, dont les observations de prix sont en EUR par
        // construction (le collecteur ne propose reference_fr que sur une grille de sièges en EUR).
        const nonEurSymbols = symbols.filter((x) => x !== "€");
        const strongBundle = fails.length === 0;
        return { ok: strongBundle, fails, corroboration: { tva_marker: markers.includes("TVA"), non_eur_symbols: nonEurSymbols } };
      },
    },
  },
};
