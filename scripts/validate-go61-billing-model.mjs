...
const commercialReview = await readFile("src/components/diagnostic/CommercialAccessReview.tsx", "utf8");
...
check(
  "capture keeps billing lightweight and commercial review groups access by family",
  stackScan.includes("getToolBillingOptions") &&
    stackScan.includes("billing_options") &&
    commercialReview.includes("buildCommercialFamilies") &&
    commercialReview.includes("ContractEditor") &&
    commercialReview.includes("Comment as-tu accès à") &&
    commercialReview.includes("plusieurs accès") &&
    commercialReview.includes("Nouvel accès") &&
    !stackScan.includes("const OFFER_OPTIONS"),
  "tool capture must not block on a universal plan grid; access is grouped by family with several access lines when needed"
);
